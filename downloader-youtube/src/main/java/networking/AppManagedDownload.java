			...
			Runnable notify = new Runnable() {
				public void run() {
					...
					case RETRYING:
						...
						if (dinfoList != null) {
							for (DownloadInfo dinfo : dinfoList) {
								System.out.println("file:" + dinfoList.indexOf(dinfo) + " - " + dinfo.getState() + " " + dinfo.getException() + " delay:"
										+ dinfo.getDelay());
							}
						}
						break;
					case DOWNLOADING:
						...
						if (now - 1000 > last) {
							...
							for (DownloadInfo dinfo : dinfoList) {
								...
								System.out.println(String.format("file:%d - %s %.2f %s", dinfoList.indexOf(dinfo), videoinfo.getState(),
